...
* Customizable Serialization of method parameters
...
When calling an API method in the client, we need to serialize the parameters of this method, send it to the backend, deserialize it in the backend and then finally call the API method implementation. Each parameter itself can be serialized as defined by your implicit Serializers and Deserializers, but multiple parameters need to be combined into a single, serializable type to be send in a request. You can configure, how *method parameters* are serialized with the following options:
- `@SerializeParametersAs.CaseClass`: Automatically generate a case class representing the parameter lists of each API method: `case class _sloth_Api_fun(a: Int, b: String, c: Double)`. When sending a request, this case class will be used to hold the method parameters, is then serialized and send to the backend. The backend deserializes the request to this case class and calls the API method implementation. This is the default behaviour.
- `@SerializeParametersAs.Tuple`: Instead of generating a dedicated case class, we just use the `Tuple` type in scala. So instead of the case class, we will serialize and deserialize a tuple: `(Int, String, Double)`.
- `@SerializeParametersAs.Single`: This serialization method expects every API method to only have one parameter. It will then use this single parameter type as the data transfer object. This is the same behaviour as `Tuple` with one argument, but does not compile for more than one argument.

Example:
```scala
@SerializeParametersAs.CaseClass // or: @SerializeParametersAs.Tuple, @SerializeParametersAs.Single
trait Api {
...
```scala
// @SerializeParametersAs.CaseClass or @SerializeParametersAs.Tuple or @SerializeParametersAs.Single
// @PathName("apiName")
trait Api {
    // @PathName("funName")
    def fun(a: Int, b: String)(c: Double): F[Int]
...
* Find out how method parameters are serialized (`Serialize` annotations are taken into account, by default `@SerializeParametersAs.CaseClass` is used).
...
* If you have enabled `@SerializeParametersAs.CaseClass`, then your chosen serialization library needs to support serializing case classes, which are generated by the macro for the parameter lists of each method in the API trait.
